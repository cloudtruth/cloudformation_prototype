    ...
    class Helper
      ...
      class Response
        ...
        def send_response(method, response_url, body = nil)
          ...
          Helper.logger.debug("Sending #{method} request (body size=#{request.body.size}) to URL #{response_url}...")
        ...
        def success(reason = '', data: {})
          Helper.logger.info("Sending #{SUCCESS_STATUS} response to CloudFormation")
          status_code = send_response('PUT', Event.instance.response_url, provider_response(SUCCESS_STATUS, reason, data: data))
        ...
        def provider_response(status, reason, data: {})
          {
            Status: status,
            Reason: reason.to_s,
