    ...
    Class.new do
      ...
      self.http = ::GraphQL::Client::HTTP.new(api_url) do
        define_method :headers do |context = {}|
          { "User-Agent": "awscf/#{CloudTruth::VERSION}", "Authorization": "Bearer #{api_key}" }
        end
      end
      ...
      def parameters(searchTerm: "")
        ...
        result&.data&.viewer&.organization&.parameters&.nodes&.collect do |e|
          CloudTruth::Parameter.new(key: e.key_name, value: e.environment_value.parameter_value, secret: e.is_secret)
        end
      end

      def parameters_hash(searchTerm: "")
        Hash[parameters(searchTerm: searchTerm).collect {|p| [p.key, p.value] }]
      end
